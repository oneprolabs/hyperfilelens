...
description: Sign in to HyperFileLens and check that the Backup Wizard is available.
---
...
1. In the `Access` section of the installation output, find `HyperFileLens` and open its `URL` in your browser.
2. Switch the interface language to **English**.
3. Select **Password**, enter the `Email` and `Password` shown under `HyperFileLens`, and complete the verification challenge.
4. Select **Sign In**.
