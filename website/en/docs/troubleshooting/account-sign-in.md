...
Open the `URL` listed under `HyperFileLens` in the `Access` section of the installation output. If the console does not open:
...
- Confirm that you opened the `URL` under `HyperFileLens`, not the `Platform Ops` administration endpoint.
