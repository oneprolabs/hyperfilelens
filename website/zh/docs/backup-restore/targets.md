...
在英文界面打开 **保护 → 备份向导 → 目标存储**，选择 **添加仓库**。仓库创建后还需要分配给备份源；仅出现在仓库列表中并不代表备份配置已经使用它。
...
![Add Repository 中的对象存储服务商选择，账户信息已经模糊处理](/docs/zh/getting-started/select-huawei-cloud.png)
...
![华为云 OBS 仓库配置表单，账户、Bucket、Object Prefix 和访问凭据已经遮盖，公开 Endpoint、Region 和 SSL 设置保持可见](/docs/zh/getting-started/configure-huawei-repository.png)
...
保存后返回 **目标存储**，确认仓库 **连接状态** 为 **在线**，再在 **备份配置** 中把它分配给对应源端。

![华为云 OBS 仓库创建完成且 Connectivity 为 Online，账户、仓库名、Bucket 和 Object Prefix 已经模糊处理](/docs/zh/getting-started/huawei-repository-created.png)
