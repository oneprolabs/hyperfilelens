...
def resolve_max_public_gateways() -> int:
    ...
    # Community does not read Enterprise instance-license rows.  Keep this
    # branch explicit so a stale license cannot alter Host-only behavior.
    if provider is None:
        return int(COMMUNITY_DEFAULT_LIMITS["max_public_gateways"])
