...
def resolve_max_organizations() -> int:
    ...
    # A Host-only Community deployment must not consume an old Enterprise
    # license row left in the database after EE was removed.
    if provider is None:
        return int(COMMUNITY_DEFAULT_LIMITS["max_organizations"])
