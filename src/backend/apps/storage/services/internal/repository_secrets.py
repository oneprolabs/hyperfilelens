...
SECRET_CONFIG_FIELDS = frozenset(
    {"secret_access_key", "smb_password", "kopia_password"}
)
...
def build_credential_metadata(
    *,
    ...
    credential_payload = (
        credential_payload if isinstance(credential_payload, dict) else {}
    )
...
def build_secret_payload(
    *,
    ...
    if repository_type == Repository.Type.S3:
        secret_access_key = credential_payload.get(
        ...
        and nas_protocol == Repository.NasProtocol.SMB
    ):
        if (
            "smb_password" in credential_payload
            and str(credential_payload.get("smb_password") or "").strip() == ""
        ):
            pass
        else:
            smb_password = credential_payload.get(
...
def credential_hint(repository: Repository) -> dict:
    ...
    if credential is None:
        ...
        return {
            ...
            "type": None,
            "secret_hint": "******" if configured else "",
        }
...
def resolve_repository_secrets(repository: Repository) -> dict:
    ...
    if credential is not None:
        try:
            ...
            Exception
        ) as exc:  # pragma: no cover - defensive wrapper around crypto backend
...
def build_repository_runtime_payload(
    *,
    ...
    config = sanitize_repository_config(
        repository.config if isinstance(repository.config, dict) else {}
    )
    ...
    if repository.repo_type == Repository.Type.S3:
        if repository.status != Repository.Status.CREATED:
            raise ValidationError(
                {"repository_id": "S3 repository has not been initialized."}
            )
        return {
            "id": repository.id,
    ...
    if repository.repo_type == Repository.Type.PROXY_FS:
        if execution_target is None:
            raise ValidationError(
                {
                    "repository_id": "Proxy filesystem repository is bound to a proxy node."
                }
            )
        ...
        if int(expected_node_id or 0) != execution_target.node.id:
            raise ValidationError(
                {
                    "repository_id": "Proxy filesystem repository is bound to a different proxy node."
                }
            )
        proxy_dir = str(config.get("proxy_node_dir") or "").strip()
        if not proxy_dir:
            raise ValidationError(
                {"repository_id": "Proxy filesystem repository path is empty."}
            )
        payload = {
            "id": repository.id,
    ...
    if repository.repo_type == Repository.Type.NAS:
        if repository.bind_node_type == Repository.BindNodeType.PROXY:
            if execution_target is None:
                raise ValidationError(
                    {"repository_id": "NAS repository is bound to a proxy node."}
                )
            if execution_target.node.role != NodeRole.PROXY:
                raise ValidationError(
                    {
            ...
            if int(expected_node_id or 0) != execution_target.node.id:
                raise ValidationError(
                    {
                        "repository_id": "NAS repository is bound to a different proxy node."
                    }
                )
            ...
            if execution_target is None:
                ...
                node_id = execution_target.node.id
            subdir = (
                nas_agent_repository_subdir(node_id)
                if node_id
                else nas_proxy_repository_subdir(repository)
            )
    ...
    raise ValidationError(
        {
            "repository_id": f"Repository type {repository.repo_type} is not supported for immediate backup."
        }
    )


def scrub_secrets(
    value: Any, *, extra_values: list[str] | tuple[str, ...] | None = None
) -> Any:
    ...
    if isinstance(value, dict):
        return {
            ...
            else scrub_secrets(item, extra_values=replacements)
...
def secret_values_for_scrub(
    repository: Repository | None = None, secrets_payload: dict | None = None
) -> list[str]:
    values: list[str] = []
    if repository is not None and isinstance(repository.config, dict):
        values.extend(
            str(v)
            for k, v in repository.config.items()
            if k in SECRET_CONFIG_FIELDS and v
        )
