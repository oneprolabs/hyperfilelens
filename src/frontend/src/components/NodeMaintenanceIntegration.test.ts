...
describe('node maintenance UI integration', () => {
  ...
  it('keeps install out of the maintenance command tabs', () => {
    ...
    expect(wizard).toContain('grid-template-columns: minmax(0, 1fr) minmax(0, 1fr)')
    expect(wizard).toContain('width: 48px')
    expect(wizard).toContain('overflow-wrap: anywhere')
    expect(wizard).toContain("t('nodeLifecycle.installPathLabel')")
    expect(wizard).toContain("t('nodeLifecycle.executionLabel')")
    expect(wizard).toContain('maintenanceExecutionKind')
    expect(wizard).toContain('maintenanceModeLabel')
  ...
  it('uses readable labels and copy in the compact maintenance summary', () => {
    ...
    expect(locale).toContain("title: 'Administrator PowerShell'")
    expect(locale).toContain("title: 'PowerShell as the installed user'")
    expect(locale).toContain("title: 'Terminal with administrator access'")
    expect(locale).toContain("title: 'Terminal as the installed user'")
