...
describe('Standalone backup data browser', () => {
  ...
  it('opens a two-region drawer with independently expandable source paths', () => {
    ...
    expect(browserPage.match(/<HflHelpTip/g)).toHaveLength(10)
    expect(browserPage).toContain('v-if="data.download_reason"')
    expect(browserPage).toContain(':content="data.download_reason"')
    expect(browserPage).toContain('popper-class="snapshot-download-restriction-help-popper"')
