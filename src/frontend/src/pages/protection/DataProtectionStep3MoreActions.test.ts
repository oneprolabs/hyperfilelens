...
describe('backup wizard step 3 More Actions refresh', () => {
  ...
  it('keeps deregister and edit actions available after a failed cleanup so Force Deregister and Reset Config can retry', () => {
    const gating = sourceBetween(
      'const startBackupSubmitting',
      'const step3CanStopBackup',
    )
    ...
    expect(gating).toContain('!sourcePendingOps.isRowSelectable(row.id)')
    expect(gating).toContain('step3LifecycleActionsEnabled.value')
    ...
    // Edit actions are gated by step3SelectionEditable (which delegates to
    // step3LifecycleActionsEnabled using isRowSelectable), so they remain
    // available after a delete_failed/delete_blocked pending op.
    expect(page).toContain(':disabled="!step3SelectionEditable"')
  })
})
